...
class TaskParamContext(
    ...
    private val _fallbacks = mutableMapOf<Int, List<TaskCandidate>>()

    /** key 为该 node 展开列表内的下标，Analyze 注入 slot 时转成 TaskSlot */
    val fallbacks: Map<Int, List<TaskCandidate>> get() = _fallbacks

    /** 登记某个任务位的后备候选；主任务 append 被 core 拒绝时按序尝试 */
    fun registerFallbacks(listIndex: Int, candidates: List<TaskCandidate>) {
        if (candidates.isNotEmpty()) _fallbacks[listIndex] = candidates
    }
